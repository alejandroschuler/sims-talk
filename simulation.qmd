...
digraph {
  ...
  A [label="1. Paper Claims 🎯", fillcolor="#f88674ff"]
  B [label="2. Simulation Goals 🎯", fillcolor="#f88674ff"]
  C [label="3. Mockup Outline, Tables & Figures 📊", fillcolor="#f6cf71"]
  D [label="4. DGPs & Settings ⚙️", fillcolor="#66c5cc"]
  E [label="5. Implement ⚙️", fillcolor="#66c5cc"]
...
# 🎯 Setting Goals {background-color="#f88674ff"}
...
- Many claims can be based on a single method. Prioritize:
...
# 📊 Presentation {background-color="#f6cf71"}
...
- **D**ata-generating mechanisms: how did you simulate the data? → `DGPs`
- **E**stimands: what are the true values? → e.g. `ATE()` helper
- **M**ethods: what methods are you using? → `ESTIMATORS`
- **P**erformance measures: what metrics? → the `summarize` step and your tables/figures
:::
:::
...
# ⚙️ Implementation {background-color="#66c5cc"}
